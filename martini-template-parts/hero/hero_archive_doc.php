<section id="hero" class="hero-title">
    <section class="hero-container">
        <div class="h-100 align-items-center">
            <div id="hero-content" class="p-2 d-flex flex-column justify-content-center">
                <h1 class="p-0 mb-2"><?php _e("Le carte della Scuola", "design_scuole_italia"); ?></h1>
                <h2 class="h4 font-weight-normal"><?php echo $testo_sezione_documenti; ?></h2>
                <div class="hero-img" style="background-image: none; background-color: url('https://www.martinomartini.eu/wp-content/uploads/2023/05/wesley-tingey-snNHKZ-mGfE-unsplash.jpg');"></div>
            </div>
        </div>
    </section>
</section>